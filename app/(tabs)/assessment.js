                      ...
                      {academicQuestions[activeSubject].map((q,i)=>{
                        ...
                        return (
                          <View key={q.id} style={[styles.academicQ, answered!==undefined&&styles.academicQAnswered]}>
                            ...
                            {q.type==="likert"&&(
                              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.likertScroll}>
                                {LIKERT_OPTIONS.map((opt,li)=>(
                                  <TouchableOpacity key={li}
                                    style={[styles.likertOpt, answered===opt&&styles.likertOptSelected]}
                                    onPress={()=>setAcademicAnswer(activeSubject,q.id,opt)}
                                  >
                                    <Text style={[styles.likertOptText, answered===opt&&styles.likertOptTextSelected]}>{opt}</Text>
                                  </TouchableOpacity>
                                ))}
                              </ScrollView>
                            )}
